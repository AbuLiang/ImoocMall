<template>
              ...
              <li v-for="(item,index) in cartList" :key="index">
                <div class="cart-tab-1">
                  ...
                  <div class="cart-item-pic">
                    <img v-lazy="'/image/'+item.productImage" :key="'/image/'+item.productImage" v-bind:alt="item.productName">
                  </div>
    ...
    <Modal :modalShow="modalConfirm" @close="closeModal">
      <p slot="message">确认要删除该商品吗?</p>
      <div slot="btnGroup">
...
<script>
    ...
    export default {
        data () { // 组件可能会被多次引用，所以data必须是函数返回一个局部变量(闭包)，不能像main.js里一样是对象，以免影响别的组件
            return {
                cartList: [],
                delItem: {}, // 缓存删除的商品id
                modalConfirm: false
                // checkAllFlag: false // 商品全选标志位
            }
        },
        mounted () { // vue生命周期初始化函数，组件加载完就会调mounted() ,里面可以调用其他初始化方法
            this.init()
        },
        filters: { // 局部过滤器s  过滤器就是一个方法，接受一个值，经过处理后返回一个新的值
          currency: currency // currency就是一个处理方法/过滤器
        },
        computed: { // 里面定义的是属性不是方法，必须有return  在computed中定义的属性，不需要再在data中声明了 如 checkAllFlag: false // 商品全选标志位
          checkAllFlag () {
            return this.checkedCount === this.cartList.length
          },
          checkedCount () { // 统计选中的商品的数量
            var i = 0
        ...
        methods: {
            ...
            delCart () {
              axios.post('/users/cartDel', {
                  ...
                  let res = response.data
                  if (res.status === '0') {
                      this.modalConfirm = false // 删除成功，关闭模态框
                      this.init() // 重新加载商品列表
            ...
            checkOut () {
                if (this.checkedCount > 0) {
                  this.$router.push({ // 页面跳转，对history的封装
                    path: '/address'
                  })
                }
            }
