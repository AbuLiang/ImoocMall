        ...
        <svg style="position: absolute; width: 0; height: 0; overflow: hidden;" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            ...
            <div class="navbar">
                <div class="navbar-left-container">
                    <a href="/">
                        <img class="navbar-brand-logo" src="./../../public/image/logo.png">
                    </a>
                </div>
...
export default {
    data () {
        return {
            userName: '周科长',
            userPwd: '123456',
            errorTip: false, // 用户名和密码错误提示
            loginModalFlag: false, // 控制遮罩层
            nickName: ''
        }
    },
    computed: { // 里面定义的是属性不是方法，必须有return 在computed中定义的计算值，不需要再在data中声明了 如 nickName
        ...mapState(['cartCount'])
        // cartCount () {
    ...
    methods: {
        ...
        login () {
            ...
            axios.post('/users/login', { // 一级路由在 server/app.js下，二级路由/login 在server/routes/users.js
        ...
        getCartCount () {
            axios.get('users/getCartCount').then((response) => {
                ...
                this.$store.dispatch('initCartCount', res.result) // 利用actions封装mutations/initCartCount()
                // console.log(this.$store.getters.add)
            })
        }
    }
}
